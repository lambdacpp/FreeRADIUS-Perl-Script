...
use vars qw(%RAD_REQUEST %RAD_REPLY %RAD_CHECK $dbh $chkh $rplh $conn_valid %conf %juniper_nas_ip_map %redback_nas_ip_map %hillstone_nas_ip_map);
use Data::Dumper;
...
sub read_conf {
   ...
   $redback_nas_ip_map{'10.255.254.1'}=1;
   $redback_nas_ip_map{'10.255.254.3'}=1;
   $hillstone_nas_ip_map{'10.255.254.50'}=1;
}
...
sub authorize {
    ...
    elsif (defined  $hillstone_nas_ip_map{$RAD_REQUEST{'NAS-IP-Address'}} ) {
        $vlan = '';
        $mac = '';
    }
    else{
    ...
    if (defined  $hillstone_nas_ip_map{$RAD_REQUEST{'NAS-IP-Address'}} ) {
        # Hillstone 
        # no mac nas-ip check
        # no write data
